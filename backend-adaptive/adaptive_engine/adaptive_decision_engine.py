...
def decide_next_step(context: AdaptiveContext, current_behavior: str) -> Tuple[Dict[str, Any], List[str]]:
    ...
    if score > 0.75:
        context.current_level = min(1.0, context.current_level + 0.1)
        trace.append("score_above_0.75_increasing_level_+0.1")
    elif score < 0.50:
        context.current_level = max(0.0, context.current_level - 0.1)
        trace.append("score_below_0.50_decreasing_level_-0.1")
    else:
